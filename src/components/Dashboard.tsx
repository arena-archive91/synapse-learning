...
import {
  ...
  Shield, Lightbulb, RotateCcw, Eye, Layout
} from 'lucide-react';
...
export function Dashboard({ stats, courses, tasks, learnerModel, onNavigate, onSelectCourse, onOpenWorkspace }: DashboardProps) {
    ...
    <div className="p-4 sm:p-6 pb-24 lg:pb-6 max-w-7xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => onNavigate('tasks')} className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 text-white rounded-xl font-medium text-sm hover:from-brand-500 hover:to-brand-400 transition-all whitespace-nowrap">
      ...
      {stats.antiPassiveAlert && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-2xl border border-accent-amber/30 bg-accent-amber/5 flex items-start gap-3">
          ...
          <div>
            ...
            <button onClick={() => onNavigate('tasks')} className="mt-2 text-xs text-brand-400 hover:text-brand-300 font-medium flex items-center gap-1">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-card p-6">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <ReadinessRing value={learnerModel.overallMastery} sublabel="Built from real performance — accuracy, retrieval, and practice volume." />
            ...
            <div className="rounded-2xl border border-accent-orange/20 bg-accent-orange/5 p-5">
              ...
              <div className="space-y-2">
                {fixTasks.slice(0, 3).map(task => (
                  <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-surface-card/50">
                    <span className="text-sm">{task.courseIcon}</span>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="space-y-6">
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-card p-5">
            ...
            <div className="flex items-end gap-1.5 h-24">
              {stats.masteryTrend.map((val, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-sm transition-all duration-500" style={{ height: `${val * 1.2}%`, backgroundColor: i === stats.masteryTrend.length - 1 ? '#818cf8' : '#2a2252' }} />
            ...
            <div className="mt-3 text-center">
              ...
              <span className="text-xs text-accent-emerald ml-2">+6% this week</span>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-card p-5">
            ...
            <div className="space-y-3">
              {learnerModel.weakAreas.slice(0, 3).map(area => (
                <div key={area.concept} className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-medium">{area.concept}</span>
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-card p-5">
            ...
            <p className="text-xs text-text-tertiary mb-2">Your predicted vs actual accuracy:</p>
          ...
          <div className="rounded-2xl border border-border-subtle bg-surface-card p-5">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-center">
              <div className="p-2 rounded-lg bg-surface-primary/50"><p className="text-lg font-bold text-accent-teal">{stats.reviewsDue}</p><p className="text-[9px] text-text-muted">Due today</p></div>
...
function MasteryRing({ mastery, size }: { mastery: number; size: number }) {
  ...
  return (
    <svg width={size} height={size} className="-rotate-90">
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="#2a2252" strokeWidth={3} />
