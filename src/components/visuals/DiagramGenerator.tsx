...
export function SupplyDemandDiagram({ priceFloor, priceCeiling }: { priceFloor?: number; priceCeiling?: number }) {
  const w = 320, h = 240, pad = 40;
  const gw = w - 2 * pad, gh = h - 2 * pad;

  // Equilibrium
  const eqX = pad + gw * 0.5, eqY = pad + gh * 0.45;

  return (
    <div className="rounded-xl border border-border-subtle bg-surface-card p-4">
      <p className="text-xs font-semibold mb-2 text-text-secondary">📊 Supply & Demand</p>
      <svg width={w} height={h} className="block mx-auto">
        {/* Axes */}
        <line x1={pad} y1={pad} x2={pad} y2={h - pad} stroke="#4d4870" strokeWidth={1.5} />
        <line x1={pad} y1={h - pad} x2={w - pad} y2={h - pad} stroke="#4d4870" strokeWidth={1.5} />
        <text x={pad - 8} y={pad + gh / 2} textAnchor="middle" fill="#706b8f" fontSize={10} transform={`rotate(-90, ${pad - 8}, ${pad + gh / 2})`}>Price</text>
        <text x={pad + gw / 2} y={h - pad + 16} textAnchor="middle" fill="#706b8f" fontSize={10}>Quantity</text>

        {/* Demand curve (downward) */}
        <motion.line
          x1={pad + 10} y1={pad + 10} x2={w - pad - 10} y2={h - pad - 10}
          stroke="#818cf8" strokeWidth={2.5} strokeLinecap="round"
          initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1 }}
        />
        <text x={w - pad + 5} y={h - pad - 5} fill="#818cf8" fontSize={10} fontWeight="600">D</text>

        {/* Supply curve (upward) */}
        <motion.line
          x1={pad + 10} y1={h - pad - 10} x2={w - pad - 10} y2={pad + 10}
        ...
        />
        <text x={w - pad + 5} y={pad + 15} fill="#2dd4bf" fontSize={10} fontWeight="600">S</text>

        {/* Equilibrium point */}
        <motion.circle
          cx={eqX} cy={eqY} r={5} fill="#fbbf24"
          initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.8, type: 'spring' }}
        />
        <text x={eqX + 10} y={eqY - 8} fill="#fbbf24" fontSize={9} fontWeight="600">E (P*, Q*)</text>

        {/* Dashed equilibrium lines */}
        <line x1={pad} y1={eqY} x2={eqX} y2={eqY} stroke="#fbbf24" strokeWidth={1} strokeDasharray="4,3" />
        <line x1={eqX} y1={eqY} x2={eqX} y2={h - pad} stroke="#fbbf24" strokeWidth={1} strokeDasharray="4,3" />

        {/* Price ceiling */}
        {priceCeiling && (
          <>
            <line x1={pad} y1={pad + gh * (1 - priceCeiling / 100)} x2={w - pad} y2={pad + gh * (1 - priceCeiling / 100)} stroke="#fb7185" strokeWidth={1.5} strokeDasharray="6,3" />
            <text x={w - pad + 5} y={pad + gh * (1 - priceCeiling / 100) + 4} fill="#fb7185" fontSize={8}>Ceiling</text>
          </>
        )}
        ...
        {priceFloor && (
          <>
            <line x1={pad} y1={pad + gh * (1 - priceFloor / 100)} x2={w - pad} y2={pad + gh * (1 - priceFloor / 100)} stroke="#fb923c" strokeWidth={1.5} strokeDasharray="6,3" />
            <text x={w - pad + 5} y={pad + gh * (1 - priceFloor / 100) + 4} fill="#fb923c" fontSize={8}>Floor</text>
          </>
        )}
      </svg>
    </div>
